...
class PlayableGame:
    ...
    def process_event(self, event: Event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            self.video_size = (
                scale * self.video_size[0], scale * self.video_size[1])
...
def play(
    ...
    keys_to_action: Optional[Dict[Union[Tuple[Union[str,
                                                    int]], str], ActType]] = None,
    ...
    for key_combination, action in keys_to_action.items():
        key_code = tuple(
            sorted(ord(key) if isinstance(key, str)
                   else key for key in key_combination)
        )
    ...
    while game.running:
        ...
        if done:
            ...
            action = key_code_to_action.get(
                tuple(sorted(game.pressed_keys)), noop)
            prev_obs = obs
            obs, rew, terminated, truncated, info = env.step(action)
            done = terminated or truncated
            if callback is not None:
                callback(prev_obs, obs, action, rew,
...
class PlayPlot:
    ...
    def __init__(
        ...
        for axis, name in zip(self.ax, plot_names):
            axis.set_title(name)
        self.t = 0
        self.cur_plot: List[Optional[plt.Axes]] = [
            None for _ in range(num_plots)]
